...
import { classService, Post, ClassFullInfo, Material } from '../../services/classService';
...
const PostItem: React.FC<PostItemProps> = ({ post, allPosts = [], onCommentPress, isDetailScreen = false }) => {
  const [userInfo, setUserInfo] = useState<any>(null);
  const [isLoadingUser, setIsLoadingUser] = useState(true);

  // Calculate comment count for this post
  const commentCount = allPosts.filter((p: Post) => p.parent_id === post.id).length;
  ...
  const getAvatarText = () => {
    ...
    if (displayName === 'Loading...' || displayName === 'Unknown User') return '?';
  ...
  const renderMediaPreview = () => {
    ...
    const mediaFiles = post.materials.filter(
      (material: Material) => material.type === 'image' || material.type === 'video'
    );

    console.log('Post', post.id, 'has', mediaFiles.length, 'media files:', mediaFiles);
          ...
          return (
            <View key={material.material_id} style={styles.mediaItem}>
              <Image
                ...
                resizeMode="cover"
                onError={(error) => {
                  console.error('Image load error for', imageUri, ':', error.nativeEvent.error);
                }}
                onLoad={() => {
