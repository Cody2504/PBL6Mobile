...
import { classService, FileUpload } from '../../services/classService';
...
const CreatePostModal: React.FC<CreatePostModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Files:', selectedFiles.map(f => ({ name: f.name, size: f.size, type: f.mimeType })));
  ...
  const handlePickFiles = async () => {
    try {
      ...
      if (selectedFiles.length + newFiles.length > MAX_FILES) {
        Alert.alert('Limit Exceeded', `You can only attach up to ${MAX_FILES} files`);
        return;
      }
      ...
      if (oversizedFile) {
        Alert.alert('File Too Large', `${oversizedFile.name} exceeds 50 MB limit`);
        return;
      }
