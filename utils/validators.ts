...
export function validatePassword(
  ...
  const { minLength = 6, requireUppercase, requireLowercase, requireNumber, requireSpecial } = requirements;
 ...
 */
export function validateOTP(code: string, length: number = 6): ValidationResult {
  if (!code || code.trim() === '') {
...
export type FieldValidators = Record<string, (value: unknown) => ValidationResult>;
