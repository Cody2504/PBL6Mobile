...
export const profileService = {
  ...
  async getUserProfilesMatchEmails(data: UserEmailsRequest): Promise<UserProfile> {
    return apiClient.post('/users/get-list-profile-match-email', data);
  },

  async updateUserProfile(userId: number, data: UpdateProfileRequest): Promise<UpdateProfileResponse> {
    return apiClient.patch(`/users/${userId}/profile`, data);
  },

  async getUsers(page: number = 1, limit: number = 10, text?: string): Promise<UsersListResponse> {
    ...
    try {
      const response: ApiUsersListResponse = await apiClient.get(`/users/list?${params.toString()}`);
