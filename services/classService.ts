...
class ClassService {
  ...
  async joinClassWithCode(classCode: string, data: JoinClassRequest): Promise<void> {
    try {
      const response = await apiClient.post(`/classes/${classCode}/joinclass`, data);
      return response.data;
    } catch (error) {
  ...
  async getClassesByUserRole(role: 'teacher' | 'student', userId: number): Promise<Class[]> {
    try {
  ...
  async uploadPostWithFiles(classId: number, data: UploadPostWithFilesRequest): Promise<UploadPostWithFilesResponse> {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/classes/${classId}/upload-post-with-files`, {
        method: 'POST',
        headers: {
  ...
  async getMaterials(classId: string): Promise<Material[]> {
    try {
      const response = await apiClient.get(`/classes/${classId}/get-all-materials`);
      return response.data;
    } catch (error) {
